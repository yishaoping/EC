...
#table(
  ...
  [`performance.c/.h`], [cycle/CSR 指令计数，以及由性能模式控制的 GHE debug counter 快照。],
...
- 每个 checker 在最终完成状态后、`ghe_release()` 前读取并发布本地统计；
...
- 发布顺序是写结果、`fence rw,rw`、写 ready；hart 0 看到全部 ready 后再
  执行 fence 并读取结果，超时 hart 显示 `not-ready`；
...
- dump 只反汇编 trap、主/次入口、checker、CLINT、GHT、负载和性能函数；
...
#table(
  ...
  [`.riscv` 文件], [`48,264 B`], [`17,976 B`], [保留 FESVR 必需符号，并包含统一 store 报告的 128 位换算/格式化支持],
  [ELF `.text`], [`11,826 B`], [`10,982 B`], [包含 debug selector 快照、store high-low-high 读取和十进制 128 位输出],
  [ELF `.bss`], [`2,080 B`], [`824 B`], [包含五 hart 的 store 结果/ready 槽和 4 个 checker debug 快照],
  [`.dump`], [`215,237 B / 4,522 行`], [`107,445 B / 2,713 行`], [保留启动、HTIF syscall 和三个 store 汇总函数],
)
...
#table(
  ...
  [卡在 GHT 完成轮询], [GH_BUF/GHM 反压、checker 状态、RSU snapshot、ICSL 窗口计数和 ELU 队列。],
  [反复触发 MTIP], [`mtimecmp` 地址、`mtime` tick、hart 数、定时器上限和 MIE/MTIE 位。],
  [固定地址访问异常], [`0x81000000..0x810008ff` 是否属于可访问且保留的 DRAM。],
  [dump 缺少函数], [该函数可能被内联或 `--gc-sections` 删除；先确认主流程是否实际引用。],
