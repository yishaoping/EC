...
class R_ELUIO(params: R_ELUParams) extends Bundle {
  ...
  val ld_valid   =  Output(Bool())
  val st_valid   =  Output(Bool())
  val req_addr   =  Output(UInt(params.wAddr.W))
  val error_ld   =  Output(Bool())
  val error_st   =  Output(Bool())
  val error_log  =  Output(UInt())
...
class R_ELU (val params: R_ELUParams) extends Module with HasR_ELUIO {
  ...
  io.ld_valid                := ld_valid
  io.st_valid                := st_valid
  io.req_addr                := req_addr_reg
  io.error_ld                := err_ld
  io.error_st                := err_st
  io.error_log               := err_log
  ...
  dontTouch(err_ld)
  dontTouch(err_st)
  dontTouch(req_addr_reg)
  dontTouch(req_data_reg)
  dontTouch(req_data_wire)
  dontTouch(resp_data_wire)
