...
class R_RSUSL(val params: R_RSUSLParams) extends Module with HasR_RSUSLIO {
  ...
  rsu_status                                     := Mux((io.clear_ic_status.asBool && (packet_index_ECP =/= 0x20.U)) || (if_check_completed.asBool && (packet_index =/= 0x20.U)), 0.U, Mux(packet_index === 0x20.U && io.check_priv === 0.U, 1.U, Mux((packet_index_ECP === 0x20.U) && !has_ECP, 3.U, rsu_status)))
