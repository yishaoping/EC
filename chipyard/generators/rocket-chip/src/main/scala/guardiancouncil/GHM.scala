...
//==========================================================
    ...
    // Track previous ic_counter values to detect falling edges (non-zero → zero)
    // so that zero values are also transmitted through the CDC
    val ic_counter_prev = RegInit(VecInit(Seq.fill(params.number_of_little_cores)(0.U(16.W))))
    ...
    for (i <- 0 to params.number_of_little_cores - 1) {
      ...
      b_wctrl(i)                     := Cat(io.ic_counter((i+2)*16-1,(i+1)*16),io.ghm_status_in(31),io.ghm_status_in(4,0))
      ic_counter_prev(i)             := io.ic_counter((i+2)*16-1,(i+1)*16)
      ...
      u_b2l_ctrl_cdc(i).io.enq.valid := io.ghm_status_in(31)|io.ghm_status_in(4,0)=/=0.U|io.ic_counter((i+2)*16-1)=/=0.U|(ic_counter_prev(i)=/=0.U)
