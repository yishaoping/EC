...
class GHT_FILTER_PRFS_IO (params: GHT_FILTER_PRFS_Params) extends Bundle {
  ...
  val ght_csr_addr_in                           = Input(UInt(12.W))
...
class GHT_FILTER_PRFS (val params: GHT_FILTER_PRFS_Params) extends Module with HasGHT_FILTER_PRFS_IO
{
  ...
  val csr_addr_in                               = RegInit(0.U(12.W))
  ...
  if (params.use_prfs){
    ...
    csr_addr_in                                  := io.ght_csr_addr_in
  ...
  //这里是分支类型
    ...
    val ght_prfs_forward_prf                      = MuxCase(0.U,
                                                      Array((dp_sel === 0.U)  -> false.B,
                                                            ((dp_sel === 1.U) && (!(csr_addr_in).isOneOf(csrshadow_seq)))  -> true.B,
                                                            (dp_sel === 2.U)  -> true.B,
                                                            (dp_sel === 3.U)  -> false.B,
                                                            (dp_sel === 5.U)  -> true.B
                                                            )
    ...
    //控制故障注入的频率，可以采用LFSR替代？
    fi_counter_tiny                              := Mux(io.gtimer_reset.asBool, 0.U, 
                                                    Mux(incr_fi_counters.asBool && !end_of_fi.asBool, fi_counter_tiny + 1.U, fi_counter_tiny))
    fi_counter                                   := Mux(io.gtimer_reset.asBool, 0.U, 
                                                    Mux(incr_fi_counters.asBool && !end_of_fi.asBool && fi_counter_tiny === 31.U, fi_counter + 1.U, fi_counter))
    ...
    io.packet_out                                := MuxCase(0.U, 
                                                      Array((dp_sel_reg === 0.U) -> 0.U,
                                                            ...
                                                            ((dp_sel_reg === 1.U) && (!(csr_addr_in_delay).isOneOf(csrshadow_seq))) -> Mux((inst_index_reg =/= 0.U), Mux(fi, fi_dp3, nfi_dp3), 0.U),
                                                            )
                                                          )
