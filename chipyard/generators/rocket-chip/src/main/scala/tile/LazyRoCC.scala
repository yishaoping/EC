...
class RoCCCoreIO(implicit p: Parameters) extends CoreBundle()(p) {
 ...
 //===== GuardianCouncil Function: Start ====//
  ...
  val traffic_counter_in = Input(Vec(GH_GlobalParams.GH_TRAFFIC_COUNTERS, UInt(64.W)))
  ...
  val debug_perf_ctrl = Output(UInt(GH_GlobalParams.GH_PERF_CTRL_BITS.W))
...
class RoccCommandRouter(opcodes: Seq[OpcodeSet])(implicit p: Parameters)
 ...
 //===== GuardianCouncil Function: Start ====//
    ...
    val traffic_counter_in = Input(Vec(GH_GlobalParams.GH_TRAFFIC_COUNTERS, UInt(64.W)))
    val traffic_counter_out = Output(Vec(GH_GlobalParams.GH_TRAFFIC_COUNTERS, UInt(64.W)))
    val ghe_packet_in = Input(UInt(GH_GlobalParams.GH_WIDITH_PACKETS.W))
    val ghe_status_in = Input(UInt(32.W))
    val bigcore_comp  = Input(UInt(3.W))
    ...
    val debug_perf_ctrl_out = Output(UInt(GH_GlobalParams.GH_PERF_CTRL_BITS.W))
    val debug_perf_ctrl_in = Input(UInt(GH_GlobalParams.GH_PERF_CTRL_BITS.W))
    val record_and_store_out = Output(UInt(2.W))
