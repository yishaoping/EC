...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  */
  ...
  /*
   * R_ICSL's mode outputs can fall before a store finishes replaying.  Use the
   * complete checker session, extended by one cycle for the registered WB
   * boundary, so normal and privileged drain states are both covered.
   */
  val storeCommitInCheckState = icsl.io.store_count_active ||
    RegNext(icsl.io.store_count_active, false.B)
  val storeAtWriteback = wb_reg_valid && wb_ctrl.mem &&
    wb_ctrl.mem_cmd.isOneOf(M_XWR, M_PWR) && !wb_xcpt && !check_exception

  /*
   * A checker store can remain at WB for many replay cycles.  Count its first
   * arrival, including a replaying arrival, but suppress retries until that
   * PC/instruction pair reaches a non-replay WB cycle.  Clearing on session
   * exit prevents a stale replay tag from affecting the next checkpoint.
   */
  val storeReplayTag = Cat(wb_reg_pc, wb_reg_inst)
  val pendingStoreReplay = RegInit(false.B)
  val pendingStoreReplayTag = RegInit(0.U((vaddrBitsExtended + 32).W))
  val repeatedStoreReplay = pendingStoreReplay &&
    pendingStoreReplayTag === storeReplayTag
  val storeCommitValid = storeCommitInCheckState && storeAtWriteback &&
    !repeatedStoreReplay

  when (!storeCommitInCheckState) {
    pendingStoreReplay := false.B
  }.elsewhen (storeAtWriteback) {
    when (replay_wb) {
      pendingStoreReplay := true.B
      pendingStoreReplayTag := storeReplayTag
    }.otherwise {
      pendingStoreReplay := false.B
    }
