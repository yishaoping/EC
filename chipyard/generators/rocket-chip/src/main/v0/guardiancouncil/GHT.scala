...
class GHT_IO (params: GHTParams) extends Bundle {
  val ght_packet_out                            = Output(UInt((GH_GlobalParams.GH_TOTAL_PACKETS*(params.packet_size)).W))
  ...
  val ght_csr_addr                              = Input(Vec(params.core_width, UInt(12.W)))
...
class GHT (val params: GHTParams) extends Module with HasGHT_IO
{
  ...
  val ght_csr_addr_ft                            = WireInit(VecInit(Seq.fill(params.core_width)(0.U(12.W))))

  for (i<- 0 to params.core_width - 1){
    new_commit_ft(i)                            := Mux((io.ght_mask_in === 1.U), 0.U, this.io.new_commit(i))
    ght_alu_in_ft(i)                            := Mux(((io.ght_mask_in === 1.U) && !(io.ght_prfs_forward_ldq.reduce(_ || _)) && !(io.ght_prfs_forward_stq.reduce(_ || _))), 0.U, this.io.ght_alu_in(i))
    ...
    ght_csr_addr_ft(i)                          := Mux((io.ght_mask_in === 1.U), 0.U, this.io.ght_csr_addr(i))
  }
  ...
  for (i <- 0 to params.core_width - 1) {  
    ...
    u_ght_filters.io.ght_csr_addr_ft(i)         := ght_csr_addr_ft(i)
