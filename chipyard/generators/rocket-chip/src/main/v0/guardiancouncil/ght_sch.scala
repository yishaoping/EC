...
{
  val current_dest                               = RegInit(0.U(4.W))
  val checker_ids                               = 1 to params.totalnumber_of_checkers

  def checkerAvailable(id: Int): Bool = {
    (id.U >= io.core_s) && (id.U <= io.core_e) && !io.core_na(id - 1).asBool
  }

  /*
   * Search from the checker after current_dest and wrap at the end.  The old
   * implementation always scanned checker IDs from 1 upward, so a frequently
   * available low-numbered checker received most checkpoints.  Each generated
   * MuxCase below has a different priority order, implementing round-robin
   * arbitration while still skipping unavailable or inactive checkers.
   */
  val first_available                           = MuxCase(0.U(4.W), checker_ids.map { id =>
    checkerAvailable(id) -> id.U(4.W)
  })
  val round_robin_dest                          = MuxLookup(current_dest, first_available,
    checker_ids.map { current_id =>
      val search_order = ((current_id + 1) to params.totalnumber_of_checkers) ++ (1 to current_id)
      current_id.U(4.W) -> MuxCase(0.U(4.W), search_order.map { id =>
        checkerAvailable(id) -> id.U(4.W)
      })
    }
  )
  val current_available                         = MuxCase(false.B, checker_ids.map { id =>
    (current_dest === id.U(4.W)) -> checkerAvailable(id)
  })
  val selected_dest                             = Mux(io.rst_sch.asBool, first_available,
                                                     Mux(current_available, current_dest, round_robin_dest))

  when (io.rst_sch.asBool) {
    current_dest                               := first_available
  }.elsewhen (!current_available && (round_robin_dest =/= 0.U)) {
    current_dest                               := round_robin_dest
  }

  io.sch_dest                                  := Cat(0.U(1.W), selected_dest)
  io.core_d                                    := 0.U
  io.sch_hang                                  := 0.U
  dontTouch(round_robin_dest)
}
