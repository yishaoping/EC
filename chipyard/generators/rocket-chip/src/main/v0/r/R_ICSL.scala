...
class R_ICSLIO(params: R_ICSLParams) extends Bundle {
  ...
  // High for the complete checker session, including exception and drain states.
  val store_count_active                         = Output(Bool())
...
class R_ICSL (val params: R_ICSLParams) extends Module with HasR_ICSLIO {
  ...
  switch (fsm_state) {
    ...
    is(fsm_postchecking_priv){
      ...
      icsl_checkerpriv_mode                     := 0.U
      /*
       * A privileged replay has already finished when it reaches this state.
       * Return to pc_special without applying another xRET.  Keep the request
       * low while the checker services a local trap so Rocket's rising-edge
       * redirect logic retries after the handler returns.
       */
  ...
  /*
   * check_priv_ret emulates the xRET at the end of a non-empty privileged
   * replay.  It must never fire in postchecking_priv: mode_ret snapshots can
   * contain the already-returned CSR state (MPP has already been consumed),
   * so applying xRET again drops the checker to U-mode.  A zero-length replay
   * also has no xRET instruction to emulate and returns through pc_special.
   */
  val privileged_replay_has_work                = ic_counter_shadow =/= 0.U
  io.if_check_privret := io.if_correct_process.asBool && RegNext((io.new_commit.asBool && ((sl_counter + 1.U) >= (ic_counter_shadow)))) && 
                         ic_counter_done.asBool && privileged_replay_has_work && (fsm_state === fsm_checking_priv) &&
                         !(io.returned_to_special_address_valid.asBool) && !io.excpt_mode
  dontTouch(check_done)
  dontTouch(privileged_replay_has_work)

  when (fsm_state === fsm_postchecking_priv) {
    assert(!io.if_check_privret, "R_ICSL issued a second privilege return after privileged checking completed")
  }

  // when()
  // exec_last_one                                 := Mux((fsm_state===fsm_speed_check),Mux(io.big_complete,true.B,exec_last_one),false.B)
  // dontTouch(exec_last_one)
  // if_rh_cp_pc is asserted only by the two postchecking states.  Do not gate
  // it with icsl_checkermode: privileged postchecking intentionally clears
  // both checker-mode outputs before redirecting to pc_special.
  if_ret_special_pc                             := if_rh_cp_pc
  ...
  io.store_count_active                         := (fsm_state === fsm_checking) ||
                                                    (fsm_state === fsm_checking_priv) ||
