...
class R_ICIO(params: R_ICParams) extends Bundle {
  ...
  val priv_higher                                = Input(Bool())
  val priv_lower                                 = Input(Bool())
  ...
  val icsl_na                                    = Input(Vec(params.totalnumber_of_cores, UInt(1.W))) // FIFO快满了
  ...
  val icsl_na_ack                                = Output(Vec(params.totalnumber_of_cores, Bool()))
  val big_checker_switch                         = Output(Vec(params.totalnumber_of_cores, Bool()))//大核事件导致切换
...
class R_IC (val params: R_ICParams) extends Module with HasR_ICIO {
  ...
  /*
   * A checker remains unavailable for a new checking session after postcheck
   * until it acknowledges the completed counter.  postcheck clears ic_status
   * before clear_ic_status has crossed back from the checker, so using
   * ic_status alone can schedule a new CPS while the previous 0x8000 completion
   * value is still outstanding.  Odd ctrl values (1/3/5) are ECP/termination
   * snapshots for the current session and intentionally keep that value set.
   */
  val checker_unavailable                       = Wire(Vec(params.totalnumber_of_cores, UInt(1.W)))
  checker_unavailable(0)                        := 1.U
  for (i <- 1 until params.totalnumber_of_cores) {
    checker_unavailable(i)                      := (ic_status(i).asBool ||
                                                     ic_counter(i)(params.width_of_ic - 1)).asUInt
  }
  ...
  for (i <- 1 to params.totalnumber_of_cores - 1) {
    u_sch_fp.io.core_na(i-1)                   := checker_unavailable(i)
  }
  ...
  val if_t_and_na                               = Mux(((io.ic_exit_isax.asBool || io.mode_switch || io.mode_ret || (ic_counter(crnt_target) >= io.ic_threshold) || io.icsl_na(crnt_target).asBool) && checker_unavailable(sch_result).asBool), 1.U, 0.U)
  val if_t_and_a                                = Mux(((io.ic_exit_isax.asBool || io.mode_switch || io.mode_ret || (ic_counter(crnt_target) >= io.ic_threshold) || io.icsl_na(crnt_target).asBool) && !checker_unavailable(sch_result).asBool), 1.U, 0.U)
  ...
  switch (fsm_state) {
    ...
    is (fsm_sch){ // 010
      ...
      crnt_target                               := crnt_target
      crnt_mask                                 := crnt_mask
      nxt_target                                := Mux(!checker_unavailable(sch_result).asBool && io.if_correct_process.asBool, sch_result, nxt_target)
      if_filtering                              := 0.U
      ...
      fsm_state                                 := Mux(!checker_unavailable(sch_result).asBool && io.if_correct_process.asBool, fsm_cooling, fsm_sch)
    }
    ...
    is (fsm_snap){ // 100
      ...
      fsm_state                                 := Mux((ctrl === 2.U) || (ctrl === 0.U) || (ctrl === 4.U), Mux(io.if_ready_snap_shot.asBool && io.if_correct_process.asBool, fsm_trans, fsm_snap), Mux(io.if_ready_snap_shot.asBool, fsm_trans, fsm_snap))
    }
    ...
    is (fsm_check){ // 110
      ...
      ctrl                                      := Mux(io.ic_exit_isax.asBool, 3.U, Mux(if_t_and_na.asBool && !(io.mode_switch || io.mode_ret || io.excp_mode), 1.U, Mux(if_t_and_na.asBool && (io.mode_switch || io.mode_ret || io.excp_mode), 5.U, Mux((io.mode_switch || io.mode_ret || io.excp_mode) && if_t_and_a.asBool, 4.U, Mux(if_t_and_a.asBool, 0.U, ctrl)))))
      crnt_target                               := crnt_target
      crnt_mask                                 := crnt_mask
      nxt_target                                := nxt_target 
  ...
  for (i <- 1 to params.totalnumber_of_cores - 1){
    nocore_available                             = nocore_available & checker_unavailable(i)
  }
  ...
  if_blocked_bySched                            := ((fsm_state === fsm_sch) && io.if_correct_process.asBool) && checker_unavailable(sch_result).asBool
  ...
  io.shared_CP_CFG                              := Mux(((fsm_state === fsm_sch) && !checker_unavailable(sch_result).asBool && io.if_correct_process.asBool && (ctrl === 0.U)), Cat(one, cp_bitmap, core_bitmap), 0.U)

  val starts_new_checker_session                 = (if_dosnap.asBool || if_dosnap_priv.asBool) &&
                                                     !ctrl(0).asBool
  for (i <- 1 until params.totalnumber_of_cores) {
    when ((fsm_state === fsm_snap) && starts_new_checker_session && (crnt_target === i.U)) {
      assert(!ic_counter(i)(params.width_of_ic - 1),
        "R_IC scheduled a checker before its previous completion was acknowledged")
    }
  }
}
