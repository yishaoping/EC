...
class RocketNpu(tile: RocketTileNpu)(implicit p: Parameters) extends CoreModule()(p)
  ...
  val csr = Module(new CSRFileNpu(perfEvents, coreParams.customCSRs.decls))
  ...
  val custom_npu = RegInit(csr.io.npu_custom)
  dontTouch(custom_npu)
  ...
  if (enableCommitLog) {
    ...
    when (csr.io.trace(0).valid) {
      printf("C%d: %d [%d] pc=[%x] W[r%d=%x][%d] R[r%d=%x] R[r%d=%x] inst=[%x] DASM(%x) wb_xcpt:%d eret:%d\n",
         ...
         coreMonitorBundle.inst, coreMonitorBundle.inst, wb_xcpt, csr.io.eret)
    }
  }
