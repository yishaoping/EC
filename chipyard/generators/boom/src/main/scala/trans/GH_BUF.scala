...
class GH_BUF (val params: GH_BUF_Params)(implicit p: Parameters) extends BoomModule
{
  ...
  val is_roccwb                                 = WireInit(VecInit(Seq.fill(params.core_width)(false.B)))//uopc===uopROCC && ldst_val
  ...
  for(i <- 0 until params.core_width){
    ...
    is_roccwb(i)                               := io.commit_valids(i) && io.commit_uops(i).uopc===uopROCC && io.commit_uops(i).ldst_val
    ...
    inst_type_enc(i)                           := MuxCase(0.U, 
                                                      Seq((io.commit_valids(i)&&io.commit_uops(i).uses_ldq) -> 1.U,
                                                          ...
                                                          (io.commit_valids(i)&&is_roccwb(i)) -> 5.U,
