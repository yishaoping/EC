...
//===== GuardianCouncil Function: Start ====//
  ...
  /*
   * Track trap-handler execution from qualified, single-cycle architectural
   * events.  CSR trace.interrupt is only the interrupt bit of the retained
   * cause value, so it can remain asserted after the trap has been taken and
   * must not be used as a level-sensitive trap-entry indication.
   *
   * r_exception_nocall covers interrupts, synchronous exceptions, and ebreak;
   * ecall is added from the committed instruction stream because the CSR
   * signal intentionally excludes it.  A committed xRET leaves handler mode.
   * Trap entry has priority so an interrupt taken immediately after xRET keeps
   * the core in exception mode without exposing normal packets in between.
   */
  val trap_entry                                 = csr.io.r_exception_nocall || if_ecall.reduce(_ || _)
  val trap_return                                = if_mret_or_sret.reduce(_ || _)
  val exception_mode_test                         = RegInit(false.B)
  when (trap_entry) {
    exception_mode_test := true.B
  }.elsewhen (trap_return) {
    exception_mode_test := false.B
  }

  // A non-retrapping xRET must release the packet path on the following cycle.
  val trap_return_without_entry_d                 = RegNext(trap_return && !trap_entry, false.B)
  when (trap_return_without_entry_d) {
    assert(!exception_mode_test, "GuardianCouncil exception mode did not clear after xRET")
  }
  dontTouch(trap_entry)
  dontTouch(trap_return)
